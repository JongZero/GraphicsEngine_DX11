...
{
public:
	CLexer(std::string fileName = NULL, DWORD sizeOfBuffer = MAX_SIZE_OF_BUFFER);
	~CLexer();
...
public:
	bool Open(std::string fileName, DWORD sizeOfBuffer = MAX_SIZE_OF_BUFFER);
	void Close();
	bool Reset();

	LONG GetToken(LPSTR tokenString);
};
