...
CLexer::CLexer(std::string fileName, DWORD sizeOfBuffer)
{
	m_File = nullptr;
...
bool CLexer::Open(std::string fileName, DWORD sizeOfBuffer)
{
	// 1) FileName이 공백이면(없으면) 리턴  false;
	if (0 >= fileName.compare(""))
	{
		return false;
	}
	...
	// 1) File을 Open한다.
	m_File = CreateFileA(fileName.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
...
//Lexical Analysis
LONG CLexer::GetToken(LPSTR tokenString)
{
	// 1) 앞쪽 쓸데없는 문자찾기
	...
	else if (Char == '*')
	{
		...
		while (Char != ' ' && Char != '\t' && Char != '\r' && Char != '\n' && Char != '"' && Char != '{' && Char != '}')
		{
			tokenString[p_StringPoint] = Char;
		...
		tokenString[p_StringPoint] = NULL;

		for (int i = 0; i < NUMBER_OF_RESERVEDWORD; i++)
		{
			std::string checkStr = tokenString;
			if (0 == checkStr.compare(Token[i]))
			{
